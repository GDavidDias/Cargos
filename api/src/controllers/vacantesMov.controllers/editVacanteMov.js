...
module.exports = async(req,res)=>{
    ...
    const{id_listado_vac_mov, orden, establecimiento, obs_establecimiento, region, departamento, localidad, cargo, turno, modalidad, cupof, id_especialidad, datetime_creacion, obs_desactiva, zona} = req.body;
    ...
    try{
        const [result] = await pool.query(`UPDATE vacantes_mov SET id_listado_vac_mov='${id_listado_vac_mov}', orden='${orden}', establecimiento='${establecimiento}', obs_establecimiento='${obs_establecimiento}', region='${region}', departamento='${departamento}', localidad='${localidad}', cargo='${cargo}', turno='${turno}', modalidad='${modalidad}', cupof='${cupof}', id_especialidad='${id_especialidad}', datetime_creacion='${datetime_creacion}', obs_desactiva='${obs_desactiva}', zona='${zona}' WHERE id_vacante_mov='${idVacanteMov}'; `);
